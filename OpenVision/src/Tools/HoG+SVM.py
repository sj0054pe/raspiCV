...
#--------------------------------------------------------------------------------------------
hog = cv2.HOGDescriptor() # HoG特徴量の計算 #クラスの生成
hog.setSVMDetector(cv2.HOGDescriptor_getDefaultPeopleDetector())# 全身の人を検出(SVM)
hogParams = {'winStride': (8, 8), 'padding': (32, 32), 'scale': 1.1}
#--------------------------------------------------------------------------------------------
#--------------------------------------------------------------------------------------------
# Lucas-Kanade法のパラメータ #mayoメモ 各画素がどこに移動していたのか表示するため
#説明サイト https://www.slideshare.net/hitoshinishimura75/lucas-kanade
...
#    print("{} : {:4.1f}ms".format("detectTime", (end - start) * 1000))
    ...
    #mayoメモ レーザを使ったコントラストの効いた画像には不向き(追跡する物体の輝度は不変が前提条件だから)。一方、一般のビデオカメラなどによる動画像への適用性は高いと言えそうです。ロボットビジョンなどでも多用されている理由はこのあたりにあるのかもしれません。
